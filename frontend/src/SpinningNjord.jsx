...
async function apiGet(){
  try{
    const r=await fetch(`${API_BASE}/data`,{headers:{"x-api-key":API_KEY}});
    if(!r.ok)return null;
    return await r.json();
  }catch{return null}
}
async function apiPut(data){
  try{
    await fetch(`${API_BASE}/data`,{method:"PUT",headers:{"Content-Type":"application/json","x-api-key":API_KEY},body:JSON.stringify(data)});
    ...
    <ThemeCtx.Provider value={t}>
      ...
      <div className="min-h-screen transition-all duration-500" style={{background:t.bg}}>
        <div className="max-w-lg mx-auto p-4 pb-24">
          ...
          <div className="text-center text-xs mt-8 space-y-1" style={{color:t.foot}}><p>{njord?"Vel m\u00f8tt! \ud83d\udcaa Husk h\u00e5ndkle og godt hum\u00f8r":"Vel m\u00f8tt!"}</p><p>Laget av Fredrik Karlsen</p></div>
        </div>
